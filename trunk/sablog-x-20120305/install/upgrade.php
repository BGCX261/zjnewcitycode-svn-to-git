...
$percount = isset($_GET['percount']) ? $_GET['percount'] : '';
...
ALTER TABLE `{$db_prefix}articles` MODIFY COLUMN `attachments` tinyint(4) NOT NULL,
	...
	ADD COLUMN `alias` varchar(200) NOT NULL,
...
ALTER TABLE `{$db_prefix}comments` MODIFY COLUMN `author` varchar(40) NOT NULL,
	MODIFY COLUMN `url` varchar(75) NOT NULL,ADD COLUMN `comment_parent` int(10) NOT NULL,
...
ALTER TABLE `{$db_prefix}searchindex` MODIFY COLUMN `dateline` int(10) unsigned NOT NULL,
	...
	ADD COLUMN `expiration` int(10) unsigned NOT NULL,
...
ALTER TABLE `{$db_prefix}users` MODIFY COLUMN `articles` int(11) unsigned NOT NULL,
	...
	MODIFY COLUMN `url` varchar(75) NOT NULL,ADD COLUMN `email` varchar(40) NOT NULL,
...
INSERT INTO {$db_prefix}settings (title, value) VALUES ('comment_email_reply', '0');
INSERT INTO {$db_prefix}settings (title, value) VALUES ('jumpwww', '0');
INSERT INTO {$db_prefix}settings (title, value) VALUES ('randarticle_num', '0');
INSERT INTO {$db_prefix}settings (title, value) VALUES ('stat_code', '');
...
EOT;
	...
	if($goon){
		...
		echo '<meta HTTP-EQUIV="REFRESH" content="2;URL='.$php_self.'?action=five">';
	}
	echo '</div></body></html>';
	exit;
} elseif ($action == 'five') {
	...
	$DB->query("DROP TABLE {$db_prefix}sessions");
	echo '<p class="p2">升级成功，请进入后台重建数据以及更新缓存并仔细检查系统设置以完成整个升级过程。</p>';
	echo '<p class="p2"><a href="../cp.php">进入控制面板</a></p>';
	echo '</div></body></html>';
	exit;
} else {
?>
	<div class="install_logo"><img src="top.gif" alt="安全天使" width="500" height="50" longdesc="http://www.4ngel.net/" /></div>
	<p class="p2">感谢您选择由 <a href="http://www.4ngel.net/" target="_blank">安全天使网络安全小组</a> 开发的 <a href="http://www.sablog.net/">SaBlog-X</a> 博客程序!</p>
	<p class="p2"><span style="color:#f00;">本程序仅适用于1.6升级到2.0，不适合任何2.0测试版本升级到2.0正式版。</span></p>
	<p class="p2"><span style="color:#f00;font-weight:700;">再次提醒，升级前必须备份数据！</span></p>
	<p class="p2">升级过程完全不用人工干预,请耐心等待成功带来的喜悦.</p>
	<p class="p2"><a href="<?php echo $php_self;?>?action=first">升级数据</a></p>
<?php
}
?>
</div>
